...
def detect_parser(text: str, path: Path) -> str:
    ...
    if name.endswith(".csv"):
        return "port_list"
    if "active connections" in lower and "local address" in lower and "foreign address" in lower:
...
def parse_file(path: Path, host_id: str) -> tuple[list[Listener], str]:
    text = path.read_text(encoding="utf-8", errors="replace")
    parser_name = detect_parser(text, path)
    source = str(path)

    if parser_name == "netstat_windows":
        return parse_netstat_windows(text, host_id, source), parser_name
    if parser_name == "ss_linux":
        return parse_ss_linux(text, host_id, source), parser_name
    if parser_name == "port_list":
        return parse_port_list(text, host_id, source), parser_name
    return parse_netstat_linux(text, host_id, source), parser_name
